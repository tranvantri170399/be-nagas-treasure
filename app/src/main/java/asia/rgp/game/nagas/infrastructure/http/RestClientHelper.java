...
/**
 * Utility class for REST client operations. Provides static methods for response validation,
...
public final class RestClientHelper {
  ...
  private static <T> T validateResponse(
      ResponseEntity<T> response, String operationName, ErrorCode errorCode) {
    if (!response.getStatusCode().is2xxSuccessful()) {
      throw new ExternalServiceException(
          String.format(
              "Failed to %s: Invalid HTTP status %s", operationName, response.getStatusCode()),
          errorCode);
    }
    ...
    if (responseBody == null) {
      throw new ExternalServiceException(
          String.format("Failed to %s: Empty response from service", operationName), errorCode);
    }
  ...
  public static ExternalServiceException handleRestClientException(
    ...
    if (e instanceof ResourceAccessException accessError) {
      ...
      return new ExternalServiceException(
          String.format(
              "Service is unavailable while %s: %s", operationName, accessError.getMessage()),
          unavailableCode,
          accessError);
    }
  ...
  public static ExternalServiceException handleHttpClientError(
      HttpClientErrorException e, String operationName, ErrorCode serviceErrorCode) {
    log.error(
        "HTTP client error while {}: {} - {}", operationName, e.getStatusCode(), e.getMessage());
