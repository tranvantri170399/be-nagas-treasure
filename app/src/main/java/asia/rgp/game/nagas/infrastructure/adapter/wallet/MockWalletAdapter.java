...
public class MockWalletAdapter implements WalletPort {
    ...
    public void debit(String userId, Money amount, String transactionId) {
        ...
        log.info("[MockWallet] DEBIT | User: {} | Amount (Real): {} | Amount (Internal): -{} | TX: {}", 
                userId, amount.getAmount(), debitAmount, transactionId);

        if (currentBalance < debitAmount) {
            log.warn("[MockWallet] DEBIT FAILED | Insufficient balance for user {}", userId);
            throw new DomainException("Số dư không đủ để thực hiện vòng quay", ErrorCode.INSUFFICIENT_BALANCE);
        }
    ...
    public void credit(String userId, Money amount, String transactionId) {
        ...
        log.info("[MockWallet] CREDIT | User: {} | Amount (Real): {} | Amount (Internal): +{} | TX: {}", 
                userId, amount.getAmount(), creditAmount, transactionId);
