...
public record SlotGameConfig(
  ...
  public SlotGameConfig {
    ...
    if (reelStrips == null || reelStrips.size() != cols) {
      throw new DomainException(
          "Reel strips count (%d) must match columns count (%d)"
              .formatted(reelStrips != null ? reelStrips.size() : 0, cols),
          ErrorCode.INVALID_DEFAULT_REEL_STRIP_COUNT);
    }

    if (payoutType == PayoutType.LINE && (paylines == null || paylines.isEmpty())) {
      throw new DomainException(
          "Line-based games must have at least one payline", ErrorCode.INVALID_PAYLINE_CONFIG);
    }

    if (symbols == null || symbols.isEmpty()) {
      throw new DomainException(
          "Symbol configuration cannot be empty", ErrorCode.SYMBOL_CONFIG_EMPTY);
    }
  ...
  private void validateSymbolId(Map<Integer, SlotSymbol> symbols, int id, String name) {
    if (!symbols.containsKey(id)) {
      throw new DomainException(
          name + " Symbol ID " + id + " not defined in symbols map", ErrorCode.SYMBOL_NOT_FOUND);
    }
  }
  ...
  public SlotSymbol getSymbol(int symbolId) {
    ...
    if (symbol == null) {
      throw new DomainException(
          "Symbol ID " + symbolId + " not found in config", ErrorCode.SYMBOL_NOT_FOUND);
    }
    return symbol;
  }
