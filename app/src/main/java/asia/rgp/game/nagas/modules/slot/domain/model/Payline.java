...
/**
 * [DOMAIN MODEL] Defines a winning pattern across the matrix.
 * The 'positions' array stores the row index for each column.
 * Example: [1, 1, 1, 1, 1] for a 5-column slot means the middle row across all reels.
 */
public record Payline(int id, int[] positions) {

    public Payline {
        if (positions == null || positions.length == 0) {
            throw new DomainException("Payline positions cannot be empty", ErrorCode.INVALID_PAYLINE_CONFIG);
        }
    }
