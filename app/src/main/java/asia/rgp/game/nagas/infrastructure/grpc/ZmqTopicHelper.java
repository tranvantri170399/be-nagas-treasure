 ...
 * <p>
 * Format: urn:ws:z:{zoneId}:r:{roomId}:p:{pluginId}:u:{userId}:s:{sessionId}
 * <p>
 * Compatible with WS Proxy KeyUtils pattern.
 */
public final class ZmqTopicHelper {
    ...
    /**
     * Parse any URN-like string by scanning for known tags.
     * Works with "urn:ws:z:1:s:sess_abc" OR "z:1:s:sess_abc" OR "s:sess_abc:z:1"
     */
    public static WSKey parse(String urn) {
        if (urn == null) {
            return new WSKey(Optional.empty(), Optional.empty(), Optional.empty(),
     ...
     */
    public static String build(String urn, String zoneId, String roomId,
            String pluginId, String userId, String sessionId) {
        StringJoiner joiner = new StringJoiner(":");

        if (urn != null)
            joiner.add("urn").add(urn);
        if (zoneId != null)
            joiner.add("z").add(zoneId);
        if (roomId != null)
            joiner.add("r").add(roomId);
        if (pluginId != null)
            joiner.add("p").add(pluginId);
        if (userId != null)
            joiner.add("u").add(userId);
        else if (sessionId != null)
            joiner.add("s").add(sessionId);

        return joiner.toString();
    }

    /**
     * Shorthand: build topic for a user session (most common usage).
     * Produces: urn:ws:z:{zone}:s:{sessionId}
     */
